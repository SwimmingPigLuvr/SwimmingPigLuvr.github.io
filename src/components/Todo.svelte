  ...
  <div class="parent relative z-50">
    ...
    <div
      ...
      <div
        ...
        tabindex="-1"
        class="category fixed font-input font-bold tracking-tighter text-xl px-[2rem] py-[0.3rem] top-[2.9rem] left-1/2 -translate-x-1/2 border-b-0 border-[0.75rem] border-t-white border-x-slate-400 text-white flex flex-row items-center justify-center hover:cursor-pointer bg-black space-x-3 transform transition-all duration-3000 ease-[backIn]">
        ...
        {#if listHover}
          ...
          >
            ...
            <!-- create list -->
            <button
              on:click={() => (showCreateList = !showCreateList)}
              class="glow-white">Create New List</button
            >
          </div>
          <!-- lists modal -->
          ...
          {#if showCreateList}
              <div
                ...
                class="absolute top-20 left-1/2 -translate-x-1/2 border-white border-2 bg-black flex flex-col p-6 space-y-8 font-input text-xl tracking-tighter text-white w-[20rem]">
                <form
                  ...
                  <input
                    ...
                    name="List"
                    id="List"
                    placeholder="Very Important Tasks"
                    bind:value={newList}
                    class="text-[1rem] p-2 bg-white bg-opacity-20"
                  />
                  <div
                    class="flex flex-row justify-end space-x-6 text-[0.81rem] pt-6"
                  >
                    <!-- cancel -->
                    <button
                      on:click|preventDefault={closeCreateList}
                      class="create-list-overlay">Cancel</button
                    >
                    <!-- done -->
                  ...
                  {#if todoHover[index]}
                    <div
                      ...
                      <button>
                        <img
                          on:mouseenter={() =>
                            (isHoveringFavorite[index] = true)}
                          on:mouseleave={() =>
                            (isHoveringFavorite[index] = false)}
                      ...
                      {#if isHoveringFavorite[index]}
                        <div
                          ...
                          out:slide={{ duration: 1000, easing: backInOut }}
                          class="font-input text-[1rem] text-center -tracking-widest absolute bottom-[3rem] -right-[1.5rem] px-2 w-[10rem] bg-yellow-300 border-black border-2">
                          Add to Favorites
                        </div>
                      {/if}
                      <!-- complete button -->
                      ...
                      {#if isHoveringButton[index]}
                        <div
                          in:slide={{ duration: 500, easing: backInOut }}
                          out:slide={{ duration: 1000, easing: backInOut }}
                          class="font-input text-[1rem] text-center -tracking-widest absolute w-[10rem] bottom-[3rem] -right-[1.5rem] px-2 bg-green-500 border-black border-2"
                        >
                          Complete Task
                        </div>
                      {/if}
                      ...
                      {#if isHoveringDelete[index]}
                        <div
                          in:slide={{ duration: 500, easing: backInOut }}
                          out:slide={{ duration: 1000, easing: backInOut }}
                          class="font-input text-[1rem] text-center -tracking-widest absolute bottom-[3rem] w-[10rem] -right-[1.5rem] px-2 bg-red-500 border-black border-2"
                        >
                          Delete Task
                        </div>
                      {/if}
          ...
          <div class="flex-2 flex-grow flex-shrink">
            ...
            <ul class=" space-y-2">
              {#each $completedList as item, index (item.id)}
                ...
                >
                  <div
                    class="item-info font-input-compressed -tracking-widest font-bold w-full px-8 py-2 rounded-md"
                  >
                    <p class="title text-[3rem]">
                      {item.title}
                    </p>
                    <p class="details line-through text-[1rem]">
                      {item.details}
                    </p>
                  </div>
